...
Certainly, a solution formula is given through the transformation to the Weierstrass canonical form and through \eqref{eq:ndae-exp-solution}. This however is not an explicit solution representation in so far as both the coefficients and the solution \(x\) itself had to be undertaken a transformation first.
...
\def\labelenumi{\arabic{enumi}.}
\item
  Representation of solutions as \(x=x_h+x_p\)
\item
  Additive splitting of DAEs into nilpotent and almost ODE parts:
\end{enumerate}
