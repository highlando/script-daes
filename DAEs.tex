...
\begin{JHSAYS}
The same \protect\hyperlink{def:dae-solution}{general solution concept}
applies. Basically \(x\) should be differentiable, fulfill the DAE, and,
if stated, the initial condition too.
\end{JHSAYS}
...
If we write down the transformed DAE that corresponds to the canonical form \eqref{eq:glob-can-form}, i.e.
\begin{align}
...
\]
Here, we have used the subscript to note that these \(d\) and \(s\) quantities were characteristic for the initial matrix pair \((E, A)\). Now, after this differentiation step, one can calculate the characteristic values \(d_1\), \(a_1\), \(s_1\) again for the pair \((E_1, A_1)\) which is obtained from the canonical form of Theorem \eqref{eq:glob-can-form} by replacing equations \eqref{eq:iv-gcf-spart-1}--\eqref{eq:iv-gcf-dpart} by the DAE with \((\bar E, \bar A)\) as in \eqref{eq:iv-gcf-post-diff}.
\end{corollary}
...
\begin{theorem}
\protect\hypertarget{thm:iv-diff-elim-invariant}{}{\label{thm:iv-diff-elim-invariant} }Assume that the pairs \((E, A)\) and \((\tilde E, \tilde A)\) are globally equivalent and in the global canonical form \eqref{eq:glob-can-form}. Then the pairs \((E_1, A_1)\) and \((\tilde E_1, \tilde A_1)\) that are obtained by differentiation and elimination as described in Corollary \ref{cor:iv-diff-elim} are globally equivalent too.
\end{theorem}
...
\hypertarget{dont-read-any-further}{%
\section{Don't read any further}\label{dont-read-any-further}}
