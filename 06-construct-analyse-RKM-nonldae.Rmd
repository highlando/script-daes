...
```{theorem, name="Kunkel/Mehrmann Thm. 5.16"}
Consider a semi-explicit, strangeness-free DAE as in \@ref(eq:semexp-dae-diff)-\@ref(eq:semexp-dae-alg) with a consistent initial value $(x_0, y_0)$. The time-discretization by a RKM, 
 ...
 * and fulfills the _Butcher condition_ $C(q)$ with $q\geq p+1$
...
The stage derivatives are then given via
\begin{equation}
...
\begin{equation}
\hat F_1(t,X_{ij},\dot X_{ij}) = 0, \quad \hat F_2(t,X_{ij}) = 0, \quad j=1,\dotsc,s \phantom{F_1} (\#eq:impl-sf-dae-collo)
\end{equation}
...
$$
Moreover, by comparing the coefficients, we find that 
$$
P_\ell(\sigma) = \tilde L_\ell(\sigma),
$$
which gives that $P_\ell(\gamma_m) = \delta_{\ell m}$ that is
$$
P_\ell(\gamma_m) = \sum_{j=1}^s\alpha_{j\ell}\dot L_j(\gamma) = 
\begin{cases}
1, &\quad \text{for }\ell =m \\
0, &\quad \text{otherwise} 
\end{cases}
$$
Accordingly, if we define $\mathcal A := \bigl[\alpha_{j\ell}\bigr] \in \mathbb R^{s,s}$ and 
$$
V:=\bigl[v_{mj}\bigr] = \bigl[ \dot L_j(\gamma_m) \bigr] \in \mathbb R^{s,s} 
$$
...
\begin{align*}
  ...
  &= -e_\ell^T \mathcal AV e x_i + \sum_{j=1}^se_\ell^T \mathcal AVe_jX_{ij} \\
  &= -x_i + X_{i\ell},
\end{align*}
