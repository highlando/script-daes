...
\BeginKnitrBlock{JHSAYS}<div class="JHSAYS">Knowing that one-step methods are stable, one typically examines only the consistency error for the approximation of ODEs. For DAEs, however, we will have to identify stable RKM methods.</div>\EndKnitrBlock{JHSAYS}
...
\min_{1\leq j \leq \nu}\{p, \kappa_j - \nu +2\},
\end{equation}
where $p$ is the order of the RKM when applied to ordinary differential equations.</div>\EndKnitrBlock{theorem}
