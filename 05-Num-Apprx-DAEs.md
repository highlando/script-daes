...
In practice, however, one really uses adaptive and, thus, nonuniform time grids.</div>\EndKnitrBlock{JHSAYS}
...
\BeginKnitrBlock{JHSAYS}<div class="JHSAYS">It is unfortunate that $x_i$ has been used to denote parts of the actual solution, but I hope this inconsistency can be tolerated in favor of a more pleasant and more standard notation.</div>\EndKnitrBlock{JHSAYS}
...
If the method $\phi$ is stable[^2], then an estimate on the (local) consistency error (cp. the definition of the iteration \@ref(eq:v-one-step-scheme)) like
$$
...
[^2]: *Stability* can be defined in many different ways. It basically means that small errors can be accumulated (that's why the order of convergence is one less the the order of consistency) but are not amplified by the method. See e.g. Kunkel/Mehrmann Def. 5.2.
...
\BeginKnitrBlock{theorem}\iffalse{-91-66-117-116-99-104-101-114-39-115-32-84-104-101-111-114-101-109-93-}\fi{}<div class="theorem"><span class="theorem" id="thm:v-butcher"><strong>(\#thm:v-butcher)  \iffalse (Butcher's Theorem) \fi{} </strong></span>If the coefficients $\beta_j$, $\gamma_j$, and $\alpha_{j\ell}$ fulfill the conditions
...
\BeginKnitrBlock{JHSAYS}<div class="JHSAYS">Knowing that one-step methods are stable, in what follows, we will examine only the consistency error for the approximation of DAEs.</div>\EndKnitrBlock{JHSAYS}
...
\BeginKnitrBlock{definition}<div class="definition"><span class="definition" id="def:v-kronecker-product"><strong>(\#def:v-kronecker-product) </strong></span>For two matrices $U \in \mathbb C^{m,n}$ and $V\in \mathbb C^{k,l}$, with 
$$
U = 
\begin{bmatrix}
u_{11} & \dots & u_{1n} \\
...
\BeginKnitrBlock{lemma}<div class="lemma"><span class="lemma" id="lem:v-kronecker-perfect-shuffle"><strong>(\#lem:v-kronecker-perfect-shuffle) </strong></span>For given dimensions $m$, $n$, $k$, $l$, there exist permutations matrices 
$$
...
\BeginKnitrBlock{lemma}<div class="lemma"><span class="lemma" id="lem:v-kronecker-associativity"><strong>(\#lem:v-kronecker-associativity) </strong></span>If the given matrices $R$, $S$, $U$, $V$ have compatible dimensions so that the products $UR$ and $VS$ exist, then
$$
...
In more generality, a complete iteration step of a general RKM applied to \@ref(eq:v-lin-dae-cc), can be expressed via the solution[^3] of the linear system
...
\BeginKnitrBlock{JHSAYS}<div class="JHSAYS">Generally, the numerical solution of DAEs requires implicit schemes.</div>\EndKnitrBlock{JHSAYS}
...
\BeginKnitrBlock{JHSAYS}<div class="JHSAYS">Please excuse the double use of the subscript like in $x_1$.</div>\EndKnitrBlock{JHSAYS}
...
\BeginKnitrBlock{proof}<div class="proof">\iffalse{} <span class="proof"><em>Proof. </em></span>  \fi{}Since we consider the pure consistency error, we can assume that $x_i=x(t_i)$. With that, with the Taylor expansion of the solution
$$
